...
def generate_db_from_json(manga_json, sites_json, db_name='mycheckmanga.db'):
    ...
    with con:
        ...
        cur.execute("CREATE TABLE manga(Title TEXT PRIMARY KEY, Site TEXT, Status TEXT, Url TEXT, LastChapterRead TEXT, TimeLastUpdated TEXT, TimeLastChecked TEXT, MostRecentChapter TEXT, CoverImageUrl TEXT, FOREIGN KEY(Site) references sites(SiteName))")
        cur.execute("CREATE TABLE completed(Title TEXT PRIMARY KEY, Rating INT, Review TEXT)")
        ...
        for manga_title in manga_list:
            cur.execute("INSERT INTO manga VALUES(:title,:site,:status,:url,:lastChapterRead,:timeLastUpdated,:timeLastChecked,:mostRecentChapter,:coverImageUrl)", (manga_title))
